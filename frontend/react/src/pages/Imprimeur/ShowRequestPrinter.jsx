...
const RequestDetails = () => {
  ...
  const handleMarkAsPrinted = async () => {
    try {
      const token = localStorage.getItem("userToken")
      const response = await fetch(`http://localhost:5000/api/tracking/printed/${id}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        throw new Error("Impossible de marquer la requête comme imprimée")
      }

      // Mettre à jour l'état local
      setRequest((prev) => ({ ...prev, status: "imprimé", comment }))
      alert("Requête marquée comme imprimée")
    } catch (err) {
      console.error("Erreur:", err)
            ...
            {request.status === "validé" && (
              <div className="action-section">
                ...
                <div className="action-buttons">
                  <button className="approve-button" onClick={handleMarkAsPrinted}>
                    <FaCheck /> Marqué comme imprimé
                  </button>
                </div>
              </div>
            )}
